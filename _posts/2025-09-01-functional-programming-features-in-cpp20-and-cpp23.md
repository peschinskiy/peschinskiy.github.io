...
1. [Lexer](https://github.com/peschinskiy/blogpost-json_stream_parser/blob/master/json_17.cpp#L34){:target="_blank" rel="noopener"} - a class that transforms input streams into tokens (language syntax primitives like **COLON**, **COMMA**, or **STRING** values) without grammar validation.
2. [Parsers](https://github.com/peschinskiy/blogpost-json_stream_parser/blob/master/json_17.cpp#L288){:target="_blank" rel="noopener"} - classes receiving lexer tokens and returning JSON items: strings, numbers, objects and arrays.
Parsers validate language grammar and throw an error for missing comma, non-string object key or unclosed array.
3. [Serializer](https://github.com/peschinskiy/blogpost-json_stream_parser/blob/master/json_17.cpp#L371){:target="_blank" rel="noopener"} - uses an output stream and the parser to stream out formatted JSON.
...
You can compare [initial](https://github.com/peschinskiy/blogpost-json_stream_parser/blob/master/json_17.cpp){:target="_blank" rel="noopener"} and [final](https://github.com/peschinskiy/blogpost-json_stream_parser/blob/master/json_23.cpp){:target="_blank" rel="noopener"} versions of the parser in the repository.
...
[Ranges](https://en.cppreference.com/w/cpp/ranges.html){:target="_blank" rel="noopener"} are iterable sequences and **Views** are lightweight wrappers on ranges describing subrange or some kind of transformations on range values:
...
Making `object_stream` and `array_stream` *ranges* requires `iterator` to [conform](https://www.reedbeta.com/blog/ranges-compatible-containers/#defining-range-compatible-iterators){:target="_blank" rel="noopener"} `std::input_iterator` requirements:
...
See full [commit](https://github.com/peschinskiy/blogpost-json_stream_parser/commit/08f9bd8597b670cef011ca695809c7a984124a9d){:target="_blank" rel="noopener"} implementing all necessary changes.
...
C++20 [concepts](https://en.cppreference.com/w/cpp/concepts.html){:target="_blank" rel="noopener"} will help us to differentiate these types:
...
Since `std::bind` [requires](https://en.cppreference.com/w/cpp/utility/functional/bind.html){:target="_blank" rel="noopener"} knowing argument counts and placeholders, currying offers a cleaner alternative.
...
C++20's `std::bind_front` is an [implementation](https://en.cppreference.com/w/cpp/utility/functional/bind_front.html){:target="_blank" rel="noopener"} of this idea:
...
Let's move to parsing implementation part. The goal of `object_stream::next_value()` [function](https://github.com/peschinskiy/blogpost-json_stream_parser/blob/master/json_17.cpp#L316){:target="_blank" rel="noopener"} is parsing input chars sequence like `,"<key>":<value>` into `std::pair<std::string, json>`.
...
Using C++20 monadic `std::optional` operations, now this can be [written](https://github.com/peschinskiy/blogpost-json_stream_parser/commit/f4c84b8ecc062c379531e550c69ac90e75c74777){:target="_blank" rel="noopener"} as a chain of conversions over `std::optional` stored value:
...
C++ [constexpr](https://en.cppreference.com/w/cpp/language/constexpr.html){:target="_blank" rel="noopener"} capabilities have evolved in every new standard, and C++20 and C++23 have introduced some important changes:
...
[Compiler Explorer example](https://godbolt.org/z/nc61v4631){:target="_blank" rel="noopener"}
...
[Reflection](https://herbsutter.com/2025/06/21/trip-report-june-2025-iso-c-standards-meeting-sofia-bulgaria/){:target="_blank" rel="noopener"} in C++26 is a long-awaited step toward compile-time type processing and RTTI.
Existing changes and future proposals such as [P2688 Pattern Matching](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2024/p2688r3.html){:target="_blank" rel="noopener"}  demonstrate C++'s evolution toward a modern functional and declarative language.
